...
points15u <- points.sf %>% sf::st_intersection(z15u_sf)%>%  
  as("Spatial") 

# clip of HydroRIVERS with the zone
hydro15u <- hydro.na.sf %>% sf::st_intersection(z15u_sf) %>% 
  as("Spatial")

# convert to UTM 15
hydro15u.utm<- spTransform(hydro15u,CRS("+proj=utm +zone=15 +datum=WGS84 +ellps=WGS84"))
points15u.utm<- spTransform(points15u,CRS("+proj=utm +zone=15 +datum=WGS84 +ellps=WGS84"))

points15u.utm.sf <- points15u.utm %>% st_as_sf() 
hydro15u.utm.sf <- hydro15u.utm %>% st_as_sf() 

# check if it are in the same projection:
st_crs(points15u.utm.sf)==st_crs(hydro15u.utm.sf) #TRUE

# view data in mapview
mapview::mapview(hydro15u.utm.sf, zcol="ORD_STRA",legend=TRUE, layer.name="Stream order")+
  mapview::mapview(points15u.utm.sf,col.regions="red", legend=F, layer.name="Sites")

# snap
points15u.snap <- maptools::snapPointsToLines(points15u.utm, hydro15u.utm, maxDist = NA, withAttrs = FALSE, idField = NA) %>% 
  cbind(points15u.utm@data) %>% 
  st_as_sf()

# ver no mapviews se deu certo:
mapview::mapview(hydro15u.utm.sf, zcol="ORD_STRA",legend=TRUE, layer.name="Stream order")+
  mapview::mapview(points15u.utm.sf,col.regions="red", legend=F, layer.name="Sites")+
  mapview::mapview(points15u.snap, col.regions="green",layer.name="Snapped sites")

# buffer 
buffer <- st_buffer(hydro15u.utm.sf, 10)

# select column
buffer.select.cols <- subset(buffer, select = c(ORD_STRA))

# Intersection of the snapped points with the buffer to obtain the order
points.order <- points15u.snap %>% sf::st_intersection(buffer.select.cols)
...
write.table(points.order.4, file="points.15u.csv",sep = ",") #export
